...
public class SocketHandle extends Thread{
    ...
    private void scoreResult()
    {
        try {
            ...
            refereeList.get(refereeUser).out.writeBoolean(succeed);

            refereeList.get(refereeUser).out.flush();
        } catch (IOException e) {
            e.printStackTrace();
        } catch (ClassNotFoundException e) {
            e.printStackTrace();
    ...
    private void getAthleteScore()
    {
        try {
            int no = in.readInt();
            out.writeObject(TcpType.AthleteScore.toString());
            Map<String, Float> rs = dao.getScoreByAthleteNo(no);
            out.writeObject(rs);
            out.flush();
        } catch (IOException e) {
            e.printStackTrace();
